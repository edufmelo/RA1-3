...
- **Comportamento da Keyword RES**: A instrução N RES foi implementada para recuperar valores do histórico de cálculos. É importante notar que linhas contendo exclusivamente operações de armazenamento (ex: (V MEM)) não geram um valor de retorno numérico na pilha e, portanto, não são contabilizadas no índice do histórico. Esse comportamento pode ser observado na execução do arquivo teste3.txt.